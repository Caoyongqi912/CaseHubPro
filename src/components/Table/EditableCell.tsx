...
const AntdEditableTable = ({ data, setData, ossFileList }) => {
  ...
  const onDeleteItem = (key) => {
    const newData = [...data];
    newData.splice(
      newData.findIndex((item) => key === item.key),
      1,
    );
    setData(newData);
  };
      ...
      render: (_, record) =>
        record.key === editingKey ? (
          type === 'FILE' ? (
            <Select
              style={{ width: '100%' }}
              placeholder="please select oss file"
              showSearch
              value={filepath}
              onChange={(e) => setFilepath(e)}
            >
              {ossFileList.map((v) => (
                <Option key={v.file_path} value={v.file_path}>
                  {v.file_path}
                </Option>
              ))}
            </Select>
          ...
          <a href={`${CONFIG.URL}/oss/download?filepath=${record.value}`}>
            {record.value}
          </a>
        ) : (
          record.value
